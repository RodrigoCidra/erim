 ...
 * exits on failure
 */
int erim_init(unsigned long long shmemSize, int flags, int ndomains) {
  int monpkey = 0;

  ERIM_DBM("INIT erim with shmemSize %lld flags %x", shmemSize,
	   flags);
  
  for (int i = 1; i < ndomains; i++) {
    if((monpkey = pkey_alloc(0, 0)) == -1) {
      ERIM_ERR("couldn't allocate pkey %d", monpkey);
      return 1;
    }
  }
  ...
  if(ERIM_FLAG_SWAP_STACK & flags) {
    for (int i = 1; i < ndomains; i++) {
      ERIM_DBM("Domain %d: %p", i, ERIM_DOMAIN_STACK_LOC(i)-16384);
      mapret = erim_mmap_domain(ERIM_DOMAIN_STACK_LOC(i) - 16384, 32768,
                  PROT_READ|PROT_WRITE,
                  MAP_ANONYMOUS | MAP_PRIVATE,
                  -1, 0, i);
      if(mapret == MAP_FAILED) {
        ERIM_ERR("FAILED TO ALLOCATE STACK %d", i);
        return 1;
      }
    }
  }
...
int erim_soInit() {
  erim_init(8129, ERIM_FLAG_ISOLATE_UNTRUSTED, 2);
